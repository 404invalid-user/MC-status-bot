...
module.exports = logger = {
  ...
  error: function (text) {
    ...
    if (config.errorSend) {
      axios
        .post('https://faq.mcstatusbot.site/error', { bot: errorinfo.bot, owner: process.env.OWNERID, site: process.env.DOMAIN, error: text, date: Date.now() })
        .then((s) => {
  ...
  crash: function (text) {
    ...
    axios
      .post('https://faq.mcstatusbot.site/error', { bot: errorinfo.bot, owner: errorinfo.owner, site: process.env.DOMAIN, error: text, date: Date.now() })
      .then((s) => {
