    ...
    for (const email of server.config.notifications.email.emails) {
        ...
        await emailClient
            .sendMail({
                ...
                text: strip_tags(server.config.notifications.email.content.replaceAll('[status]', server.pinger.status).replaceAll('[ip]', server.IP)) + '\n\n-----\nthis was an automated email sent from https://www.mcstatusbot.site to notify you of your minecraft server status if you do not want to receive these email please reply (only me the bot maintainer can see replys)', // plain text body
                html: strip_tags(server.config.notifications.email.content.replaceAll('[status]', server.pinger.status).replaceAll('[ip]', server.IP)) + '<br><br>-----<br>this was an automated email sent from https://www.mcstatusbot.site to notify you of your minecraft server status if you do not want to receive these email please reply (only me the bot maintainer can see theses)' // html body
            })
            .then((e) => {
...
const strip_tags = (html, ...args) => {
    return html.replaceAll(/<(\/?)(\w+)[^>]*\/?>/g, (_, endMark, tag) => {
