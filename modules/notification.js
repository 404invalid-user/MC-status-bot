...
function webhook(server, config, pinger) {
  if ((pinger.status == 'online' && config.for.online) || (server.pinger.status == 'offline' && config.for.offline)) {
    axios
      .post(config.url, {
        content: server.config.notifications.webhook.content
          .replaceAll('[status]', server.pinger.status)
          .replaceAll('[ip]', server.IP)
          .replaceAll('[name]', server.name)
          .replaceAll('[motd]', server.pinger.motd.clean)
      })
      .then((res) => {
...
function email(server, config, pinger) {
  if ((pinger.status == 'online' && config.for.online) || (server.pinger.status == 'offline' && config.for.offline)) {
    for (const email of config.emails) {
      const subject = server.config.notifications.email.subject
        .replaceAll('[status]', server.pinger.status)
        .replaceAll('[ip]', server.IP)
        .replaceAll('[name]', server.name)
        .replaceAll('[motd]', server.pinger.motd.clean)
      const body = server.config.notifications.email.content
        .replaceAll('[status]', server.pinger.status)
        .replaceAll('[ip]', server.IP)
        .replaceAll('[name]', server.name)
        .replaceAll('[motd]', server.pinger.motd.clean)
