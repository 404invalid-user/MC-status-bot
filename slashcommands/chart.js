...
module.exports = {
    ...
    async execute(interaction, server, client) {
        ...
        if (!chartType) return interaction.editReply(await translate(server.lan, 'Please specify what you want to chart! Use `/chart uptime`, `/chart playersonline` or `/chart mostactive`'));


        // Get the ip. data.IP holds the ip
        if (!server.Logging) return interaction.editReply(await translate(server.lan, 'This server has logging set to off. please ask an admin to do `/log value: on`'));
        ...
        // Check if logs exist
        if (logs.length <= 1 || logs == null || !server.IP) return interaction.editReply(await translate(server.lan, "This server doesn't have any logs, please wait for them to update!"));
        ...
        if (playersList.length > 0) {
            ...
            playersList.forEach(function(e) {
                if (numberOfOccurrences.hasOwnProperty(e)) {
                    numberOfOccurrences[e]++;
                } else {
                    numberOfOccurrences[e] = 1;
                };
            });
            // Sort it by the value
            const sorted = Object.entries(numberOfOccurrences).sort(([c1, v1], [c2, v2]) => v2 - v1).reduce((o, [k, v]) => ((o[k] = v), o), {});
        ...
        if (chartType == 'playersonline') {
            ...
            embedDescription = server.config.chart.enabled ? formatText(server.config.chart.embed.playersonline.description) : `There have been a maximum of ${Math.max(...playersOnline)} players online at once, and a minimum of ${Math.min(...playersOnline)}.`;
            ...
            embedDescription = server.config.chart.enabled ? formatText(server.config.chart.embed.mostactive.description) : `${mostActive.name} was the most active player with ${mostActive.time} minutes spent online in the last 24 hours.`;
