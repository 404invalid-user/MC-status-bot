...
module.exports = {
  ...
  execute(message, args, server) {
    ...
    if (logging == true) {
      ...
      Log.findByIdAndUpdate({ _id: message.guild.id }, { logs: [] }, { useFindAndModify: false, new: true, upsert: true }).cache().catch((err) => {
      ...
      Log.findOneAndRemove({ _id: message.guild.id }, { useFindAndModify: false, new: true }).cache().catch((err) => logger.error(err.stack || err)).then(message.channel.send(`Logging has been turned off`))
    }
