...
module.exports = {
    ...
    async run(shards, req, res) {
        try {
            ...
            if (req.body.data) return res.status(400).json({ message: '400: please supply the whole user object with the changes made', responseTime: (Date.now() - parseFloat(req.date)).toString() + 'ms' });
            ...
            if (req.user.lan != req.body.data.lan) {
                ...
                if (lanfc == null) return res.status(400).json({ message: '400: supplied lan has not been translated', responseTime: (Date.now() - parseFloat(req.date)).toString() + 'ms' });
