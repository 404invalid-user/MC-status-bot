    ...
    try {
      ...
      if (req.body.data.Logging != server.Logging) {
        ...
        if (req.body.data.Logging) {
          ...
          Log.findOne({ _id: interaction.guild.id })
            .remove()
      ...
      await server.save()
      if (beenError) return res.status(200).json({ message: '200400: success with errors', errorid: '0', responseTime: (Date.now() - parseFloat(req.date)).toString() + 'ms' })
