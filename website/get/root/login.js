    ...
    async run(shards, req, res) {
        try {
            if (req.query.code) {
                ...
                tokenData.append('scope', 'identify guilds');

                await axios.post('https://discord.com/api/oauth2/token', tokenData.toString(), {
                ...
                if (oath == undefined) return res.redirect('/dashboard');
                await axios.get('https://discord.com/api/users/@me', {
                    headers: { authorization: `${oath.token_type} ${oath.access_token}` }
                }).then((responce) => {
                ...
                cache.create('user', currentUser._id, currentUser);
                ...
                return res.redirect(`https://discord.com/api/oauth2/authorize?client_id=${process.env.client_id}&redirect_uri=${process.env.redirect_uri}&response_type=code&scope=identify%20guilds`);
            }
        } catch (error) {
