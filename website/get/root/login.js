  ...
  async run(shards, req, res) {
    try {
      if (req.query.code) {
        ...
        await axios
          .post('https://discord.com/api/oauth2/token', tokenData.toString(), {
        ...
        return res.redirect(
          `https://discord.com/api/oauth2/authorize?client_id=${process.env.client_id}&redirect_uri=${process.env.redirect_uri}&response_type=code&scope=identify%20guilds`
        )
      }
    } catch (error) {
