    ...
    async run(shards, req, res) {
        try {
            if (req.query.code) {
                ...
                await axios.post('https://discord.com/api/oauth2/token', tokenData.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }).then(responce => oath = responce.data).catch(err => {
                    logger.error(err.stack || err)
                    if (err.toString() == 'Error: Request failed with status code 400') {
                        return res.redirect('/login?ref=code-fail');
                    }
                });
                await axios.get('https://discord.com/api/users/@me', { headers: { authorization: `${oath.token_type} ${oath.access_token}` } }).then(responce => userInfo = responce.data).catch(err => {
                    logger.error(err.stack || err)
                });
                await axios.get('https://discord.com/api/users/@me/guilds', { headers: { authorization: `${oath.token_type} ${oath.access_token}` } }).then(responce => userGuilds = responce.data).catch(err => {
                    logger.error(err.stack || err)
                });
                ...
                for await (const userGuild of userGuilds) {
                    guilds.push({ id: userGuild.id, mutual: botGuilds.includes(userGuild.id) ? true : false, admin: dbc.permissions(userGuild.permissions_new).includes('MANAGE_GUILD') ? true : false });
                }
                delete botGuilds;
                ...
                res.cookie('id', currentUser._id, { expires: new Date(253402300000000), httpOnly: true }).cookie('token', randomString, { expires: new Date(253402300000000), httpOnly: true }).status(200).redirect('/');
                ...
                return res.redirect(`https://discord.com/api/oauth2/authorize?client_id=${process.env.client_id}&redirect_uri=${process.env.redirect_uri}&response_type=code&scope=identify%20email%20guilds`)
            };
        } catch (error) {
