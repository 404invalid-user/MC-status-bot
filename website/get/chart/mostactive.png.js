...
module.exports = {
    ...
    async run(shards, req, res) {
        try {
            ...
            // Check if logs exist
            if (logsraw.logs.length <= 1 || logsraw == null || !server.IP) return res.status(200).send(await translate(server.lan, "This server doesn't have any logs, please wait for them to update!"));
            ...
            const configuration = {
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: `${await translate(server.lan, 'Most Active')}`,
